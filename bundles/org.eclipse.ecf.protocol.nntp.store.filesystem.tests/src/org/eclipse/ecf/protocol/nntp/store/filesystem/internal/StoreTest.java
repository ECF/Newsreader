...
public class StoreTest {
	...
	public void setUp() throws Exception {
		...
		store = new StoreFactory().createStore(SALVO.SALVO_HOME
		...
		for (int i = 0; i < subscribedServers.length; i++) {
			...
			store.unsubscribeServer(iServer, true);
		}
	...
	public void testSubscribeNewsgroup() throws NNTPException {
		testSubscribeServer();
		INewsgroup newsgroup = NewsgroupFactory.createNewsGroup(
				store.getSubscribedServers()[0], "eclipse.platform.rcp",
	...
	public void testStoreArticles() throws NNTPIOException,
		...
		for (String string : overviewHeaders) {
			IArticle article = ArticleFactory
					.createArticle(headers, string, store
		...
		assertTrue(store
				.getArticles(store.getSubscribedNewsgroups(store
	...
	public void testStoreArticleBody() throws StoreException, NNTPIOException,
		...
		for (int i = 0; i < overviewHeaders.length; i++) {
			store.storeArticleBody(
					store.getArticle(store.getSubscribedNewsgroups(store
							.getSubscribedServers()[0])[0], i + 1), body);
	...
	public void testGetArticleBody() throws NNTPIOException,
		...
		for (int i = 0; i < overviewHeaders.length; i++) {
			String[] body = store
					.getArticleBody(store.getArticle(store
							.getSubscribedNewsgroups(store
	...
	public void testGetFollowUps() throws NNTPIOException,
		...
		for (int i = 0; i < overviewHeaders.length; i++) {
			IArticle article = store
					.getArticle(store.getSubscribedNewsgroups(store
							.getSubscribedServers()[0])[0], i + 1);
